...
const MAX_SYLLABUS_TEXT = 7000;

//const MAX_ANALYZE_TEXT   = 5000;

const CHUNK_SEARCH_K = 10;
const CHUNK_USE_K = 6;
const CHUNK_SCORE_THRESHOLD = 0.30;
...
// LESSON BUDGET
// ─────────────────────────────
//quy định chi phí viết nội dung bài giảng
...
// REGEX PATTERNS
// ─────────────────────────────────────────────
//loại bỏ quiz hoặc đáp án rác
//chặn giải thích trong đáp án
...
//đổi model kế tiếp nếu model bị giới hạn
...
//hàm quy định thời gian chờ nếu lỗi 429
...
//cổng giao tiếp với LLM-thả format hơn
...
//xử lý lỗi JSON format, format lại JSON
// ─────────────────────────────────────────────
const safeJSONParse = (text) => {
  ...
  // Extract JSON block nếu bị lẫn text,do tìm nốt mở đóng ngoặc nhọn
...
// TEXT HELPERS
// ─────────────────────────────
//hàm chuẩn hóa văn bản extracted
...
// ─────────────────────────────────────────────
//trích xuất tên biến bảng để nhắc AI dùng đúng tên
// ─────────────────────────────────────────────
...
const stripInvalidCodeBlocks = (content, contextText) => {
  ...
  while ((match = CODE_BLOCK_RE.exec(content)) !== null) {
    ...
    // ✅ FIX: Tăng threshold lên 3 (từ 2) — giảm false-positive với tài liệu phức tạp
...
//hàm lấy đoạn văn tránh đầu voi đuôi chuột
...
// ─────────────────────────────────────────────
// SCOPE GUARD — hard validation sau generate
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// ANTI-DUP (JACCARD)////hàm chống lặp bằng số liệu
// ─────────────────────────────────────────────
//bóc tách từ phổ thông
const tokenize = (text) => {
...
//tính toán mức độ tương đồng nội dung sử dụng thuật toán Jaccard
...
// ─────────────────────────────────────────────
// QUIZ VALIDATION PIPELINE (FIXED)//kiểm tra chất lượng quiz
// ─────────────────────────────────────────────

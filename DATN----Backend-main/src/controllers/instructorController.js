...
const finalizeReview = async (req, res) => {
  try {
    ...
    // Reset tiến độ của học viên về ngày 1 khi giảng viên gửi lại khoá học
    // Tìm các học viên đăng ký lộ trình này
    const enrollments = await Enrollment.find({ planId: plan._id });
    const studentIds = enrollments.map(e => e.learnerId);
    if (plan.owner && !studentIds.includes(plan.owner)) {
      studentIds.push(plan.owner);
    }

    const targetPlanIds = [plan._id];

    await Progress.updateMany(
      { planId: { $in: targetPlanIds }, userId: { $in: studentIds } },
      { 
        completedDays: [], 
        lessonScores: [], 
        averageScore: 0, 
    ...
    // Reset lesson status in database back to Day 1 = "in-progress" and Day 2+ = "locked"
    await Lesson.updateMany(
      { planId: { $in: targetPlanIds }, dayNumber: 1 },
      { status: "in-progress" }
    );
    await Lesson.updateMany(
      { planId: { $in: targetPlanIds }, dayNumber: { $gt: 1 } },
      { status: "locked" }
    );

    return res.success(null, "Đã gửi bản chỉnh sửa cho học viên và reset tiến độ về ngày 1.");
  } catch (error) {

...
// ─── getYouTubeEmbedUrl Utility ──────────────────────────────────────────────
const getYouTubeEmbedUrl = (url?: string) => {
  if (!url) return "";
  
  // RegEx trích xuất video ID từ nhiều định dạng URL YouTube khác nhau
          ...
          <div className="space-y-8 animate-in fade-in pb-20">
            ...
            {!quizResult ? (
              <div className="space-y-4">
                {quizQuestions.length === 0 ? (
                  /* ── Trạng thái: Quiz chưa có (đã bị xoá khi tạo lại nội dung) ── */
                  <div className="space-y-3">
                    {/* Info banner */}
                    <div className="flex items-start gap-3 p-4 bg-amber-500/8 border border-amber-500/25 rounded-2xl animate-in fade-in">
                      <span className="text-xl shrink-0">🔄</span>
                      <div>
                        <p className="text-amber-300 font-black text-sm">Bộ câu hỏi đã bị xoá sau khi tạo lại nội dung</p>
                        <p className="text-slate-500 text-xs mt-1 leading-relaxed">
                          Bạn có thể tạo bộ câu hỏi mới phù hợp với nội dung vừa cập nhật, hoặc bỏ qua quiz ngày này để tiếp tục.
                        </p>
                      </div>
                    </div>
                    {/* Two action buttons */}
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <button
                        ...
                        className="py-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white rounded-[1.5rem] font-black text-sm shadow-lg shadow-blue-900/30 transition-all active:scale-95 flex items-center justify-center gap-2 disabled:opacity-50"
                      >
                        <RefreshCw size={18} className={loadingPool ? 'animate-spin' : ''} />
                        Tạo lại Quiz cho ngày này
                      </button>
                      <button
                        onClick={handleSubmitQuiz}
                        disabled={submittingQuiz || loadingPool}
                        className="py-4 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 text-white rounded-[1.5rem] font-black text-sm shadow-lg shadow-amber-900/30 transition-all active:scale-95 flex items-center justify-center gap-2 disabled:opacity-50"
                      >
                        {submittingQuiz ? <Loader2 size={18} className="animate-spin" /> : <CheckCircle size={18} />}
                        Bỏ qua Quiz &amp; Mở khoá Ngày {Number(dayNumber) + 1}
                      </button>
                    </div>
                  </div>
                ) : (
                  /* ── Trạng thái: Đã có câu hỏi → nút nộp bài bình thường ── */
                  <div className="space-y-3">
                    <button
                      onClick={handleSubmitQuiz}
                      disabled={submittingQuiz || loadingPool}
                      className="w-full py-5 text-white rounded-[1.5rem] font-black text-xl shadow-2xl transition-all active:scale-95 flex items-center justify-center gap-3 disabled:opacity-50 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 shadow-blue-900/30"
                    >
